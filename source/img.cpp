...
void Mat_Mean(double *mean, Matrix *A){ //to GPU
	double tmp = 0.0;
	for(int h = 0 ; h < A->height ; h++)
		for (int w = 0 ; w < A->width ; w++)
			tmp += A->data[h][w];
...
void Mat_Vector(Matrix *A, float *a){ //to GPU
	for (int h = 0 ; h < A->height ; h++)
		for (int w = 0 ; w < A->width ; w++)
			a[h*A->width+w] = (float) A->data[h][w];
}

void Mat_Shift(Matrix *A, Matrix *B, int side){ //to GPU
	for(int h = side ; h < B->height ; h++)
		for (int w = side ; w < B->width ; w++)
...
void Mat_Zeros(Matrix *A){ //to GPU
	for(int h = 0 ; h < A->height ; h++)
		for(int w = 0 ; w < A->width ; w++)
			A->data[h][w] = 0;
}

void Mat_Zeros_uc(uc_Matrix *A){ //to GPU
	for (int h = 0 ; h < A->height ; h++)
		for(int w = 0 ; w < A->width ; w++)
			A->data[h][w] = 0;
}

void Mat_Zeros_i(i_Matrix *A){ //to GPU
	for(int h = 0 ; h < A->height ; h++)
		for(int w = 0 ; w < A->width ; w++)
			A->data[h][w] = 0;
}
...
void GaborFilteredImg(Matrix *FilteredImg_real, Matrix *FilteredImg_imag, Matrix *img, int side, double Ul, double Uh, int scale, int orientation, int flag){
	...
	int i;
    printf( "llegas?\n" );
    for(i = 0 ; i < (2 * scale * orientation) ; i++)
