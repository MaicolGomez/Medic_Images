...
void Mat_Vector(Matrix *A, float *a)
{
	...
	for (h=0;h<A->height;h++)
		for (w=0;w<A->width;w++)
			a[h*A->width+w] = (float) A->lData[h][w];
}
...
void Mat_Shift(Matrix *A, Matrix *B, int side)
{
	...
	for (h=side;h<B->height;h++)
		for (w=side;w<B->width;w++)
			A->lData[h-side][w-side] = B->lData[h][w];

	for (h=side;h<B->height;h++)
		for (w=0;w<side;w++)
			A->lData[h-side][B->width-side+w] = B->lData[h][w];

	for (h=0;h<side;h++)
		for (w=side;w<B->width;w++)
			A->lData[B->height-side+h][w-side] = B->lData[h][w];

	for (h=0;h<side;h++)
		for (w=0;w<side;w++)
			A->lData[B->height-side+h][B->width-side+w] = B->lData[h][w];
}
...
void CreateMatrix(Matrix **M, int hei, int wid)
{
    ...
    temp->lData = new double*[hei];
    for(int j=0; j<hei;j++)
        temp->lData[j] = new double[wid];
...
/*
	...
	tmp->lData = (double **) calloc(hei, sizeof(double *));
	...
	for (h=0; h<hei; h++) {
		tmp->lData[h] = (double *) calloc(wid, sizeof(double));
		if (!(tmp->lData[h])) {
			nrerror("allocation failure in CreateMatrix()");
			exit(1);
		}
...
void Create_i_Matrix(i_Matrix **M, int hei, int wid)
{
    ...
    i_Matrix *temp = new i_Matrix;


    temp->data = new int*[hei];
    for(int j=0; j<hei;j++)
        temp->data[j] = new int[wid];
...
void Create_uc_Matrix(uc_Matrix **M, int hei, int wid)
{
    ...
    uc_Matrix *temp = new uc_Matrix;

    temp->data = new unsigned char*[hei];
    for(int j=0; j<hei;j++)
        temp->data[j] = new unsigned char[wid];
	...
	ys = Input_real->width;
        ...
        for (i=1;i<=Input_real->height;i++)
            for (j=1;j<=Input_real->width;j++) {
                R[i][j] = Input_real->lData[i-1][j-1];
                I[i][j] = Input_imag->lData[i-1][j-1];
            }
        ...
        for (i=1;i<=Input_real->height;i++)
            for (j=1;j<=Input_real->width;j++) {
                Output_real->lData[i-1][j-1] = Fr[i][j];
                Output_imag->lData[i-1][j-1] = Fi[i][j];
            }
...
void Mat_IFFT2(Matrix *Output_real, Matrix *Output_imag, Matrix *Input_real, Matrix *Input_imag)
{
	...
	NN = (double) (xs*ys);
        ...
        for (i=1;i<=Input_real->height;i++)
            for (j=1;j<=Input_real->width;j++) {
                Output_real->lData[i-1][j-1] = Fr[i][j]/NN;
                Output_imag->lData[i-1][j-1] = Fi[i][j]/NN;
            }
		...
		if ((h_begin >= 0)&&(h_begin < B->height)&&(w_begin >= 0)&&(w_begin < B->width)) {
			...
			if ((h >= 1)&&(w >= 1)) {
				...
				if ((h_done < A->height)&&(w_done < A->width)) {
					for (i=0;i<h;i++) {
						for (j=0;j<w;j++) {
							A->lData[i+h_target][j+w_target] = B->lData[i+h_begin][j+w_begin];
						}
					}
				}
			}
...
void Mat_Product(Matrix *A, Matrix *B, Matrix *C)
{
	...
	for (h=0;h<A->height;h++) {
		for (w=0;w<A->width;w++) {
			A->lData[h][w] = B->lData[h][w]*C->lData[h][w];
		}
	}
}
...
void Mat_Fliplr(Matrix *A)
{
	...
	for (h=0;h<A->height;h++) {
		for (w=0;w<A->width;w++) {
			tmp->lData[h][w] = A->lData[h][(A->width)-w-1];
		}
	}

	for (h=0;h<A->height;h++) {
		for (w=0;w<A->width;w++) {
			A->lData[h][w] = tmp->lData[h][w];
		}
	}
...
void Mat_Flipud(Matrix *A)
{
	...
	for (h=0;h<A->height;h++) {
		for (w=0;w<A->width;w++) {
			tmp->lData[h][w] = A->lData[(A->height)-h-1][w];
		}
	}
